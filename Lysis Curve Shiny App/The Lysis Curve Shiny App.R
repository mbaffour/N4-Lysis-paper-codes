# Lysis Curve OD Visualization App  v2.6
...
server <- function(input, output, session) {
  ...
  # Null-coalescing operator (missing in base R < 4.4)
    ...
    if (rv$is_long_format) {
      ...
      pd <- d %>%
        ...
        summarise(mean_value = mean(.data[[rv$value_col]], na.rm = TRUE),
                  ...
                  n          = n(), .groups = "drop") %>%
        mutate(sem_value  = sd_value / sqrt(n),
  ...
  build_plot <- function(plot_data, samples, aes_vals, highlight_samples = NULL) {
    ...
    if (input$error_type != "none") {
      ...
      lo_val <- if (input$y_scale_type == "log")
        pmax(plot_data$mean_value - plot_data[[ec]] * em, 1e-9)
      else if (isTRUE(input$asymmetric_error))
        pmax(plot_data$mean_value - plot_data[[ec]] * em, 0)
      else
        plot_data$mean_value - plot_data[[ec]] * em
      plot_data$err_lo <- lo_val
      plot_data$err_hi <- plot_data$mean_value + plot_data[[ec]] * em
    ...
    if (input$show_points) {
      ...
      # Point colour is driven by the existing scale_color_manual (same as lines),
      # so we only need to resolve shape and fill here.
      # Filled shapes:  fill = line colour (via display_colors)
      # Open shapes:    fill = "white", shape remapped to stroke-only variant
      ...
      # Attach fill to the data frame; colour comes from the parent aes mapping
      # (variable → colour via scale_color_manual) so points automatically match lines.
      pt_data          <- plot_data
      ...
      p <- p + geom_point(
        ...
        scale_shape_manual(values = shape_map, labels = leg_labels, name = NULL, guide = "none") +
        scale_fill_identity()
    }
